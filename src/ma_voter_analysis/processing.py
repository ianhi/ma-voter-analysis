...
def load_voter_list(file: str, date_for_age: DatetimeScalar)->pd.DataFrame:
    ...
    ----------
    file : Pathlike
        Pathlike to the csv file
    date_for_age : Datelike
        The date to use when adding the age column.

    Returns
    -------
    pd.DataFrame
        The voter list form the file. Slightly cleaned up to drop
        voters with Birthdays well over 100 years old
    """
    vlist = load_try_encodings(file, _voter_list_dtypes)
    vlist.rename({c: c.strip() for c in vlist.columns}, inplace=True, axis=1)
    vlist = vlist.set_index("Voter ID Number")
    vlist = vlist.drop(DROPPED_IDS, errors="ignore")
...
def load_year(
    ...
    """
    Load and combine the voter history and voter list for a given year.

    Parameters
    ----------
    voter_history : Pathlike
        Path to the history csv
    voter_list : Pathlike
        Path to the voter list csv

    Returns
    -------
    pd.DataFrame
    """
    history = load_history(voter_history)
    # extract from the file this will probs break if multiple elec in same year
    elec_date = pd.to_datetime(history["Election Date"].iloc[0])
    vlist = load_voter_list(voter_list, elec_date)
